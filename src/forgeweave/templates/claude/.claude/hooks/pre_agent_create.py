...
def main():
    ...
    if not agent_id or not agent_id.replace("-", "").isalnum():
        print(json.dumps({"decision": "block", "reason": "agent_id must be kebab-case alphanumeric"}), file=sys.stderr)
        sys.exit(2)
    ...
    for pattern in ["agents/*.md", "agents/*.yaml", "agents/**/*.md"]:
        for f in Path(".").glob(pattern):
            if f.stem == agent_id:
                print(json.dumps({"decision": "block", "reason": f"Duplicate agent_id: {agent_id}"}), file=sys.stderr)
                sys.exit(2)

...
- [ ] The skill passes `forge validate skill <path>` without errors.
...
| `forge validate` returns errors | Stop. Report: "Validation failed: <errors>. Fix all validation errors before submitting." |
...
```json
{
  ...
  "existing_steps": "Current workflow: manually edit each SKILL.md, run forge validate, fix errors"
}
```
